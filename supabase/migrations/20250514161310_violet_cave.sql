/*
  ...
  1. New Functions
    - `get_zones_with_multipliers` - Returns zones with their multiplier values
    - `get_zone_multipliers` - Returns zone multiplier data without ambiguous columns
    - `update_vehicle_prices` - Updates vehicle price data with improved error handling
    - `update_zone_multipliers` - Updates zone multiplier data with improved error handling
    - `update_fixed_routes` - Updates fixed route data with improved error handling

  2. Changes
    - Drop existing functions first to avoid return type changes
    - Add better error handling for each function
    - Return structured results for update operations
*/

-- Drop existing functions if they exist
DROP FUNCTION IF EXISTS public.get_zones_with_multipliers();
DROP FUNCTION IF EXISTS public.get_zone_multipliers();
DROP FUNCTION IF EXISTS public.update_vehicle_prices(jsonb);
DROP FUNCTION IF EXISTS public.update_zone_multipliers(jsonb);
DROP FUNCTION IF EXISTS public.update_fixed_routes(jsonb);

-- Function to get zones with their multiplier values safely
CREATE FUNCTION public.get_zones_with_multipliers()
RETURNS TABLE (
  id uuid,
  name text,
  multiplier_id uuid,
...
CREATE FUNCTION public.get_zone_multipliers()
RETURNS TABLE (
  id uuid,
  zone_id uuid,
  multiplier numeric,
...
CREATE FUNCTION public.update_vehicle_prices(prices_json JSONB)
RETURNS JSONB
LANGUAGE plpgsql
SECURITY DEFINER
SET search_path = public
...
CREATE FUNCTION public.update_zone_multipliers(multipliers_json JSONB)
RETURNS JSONB
LANGUAGE plpgsql
SECURITY DEFINER
SET search_path = public
...
CREATE FUNCTION public.update_fixed_routes(routes_json JSONB)
RETURNS JSONB
LANGUAGE plpgsql
SECURITY DEFINER
SET search_path = public
